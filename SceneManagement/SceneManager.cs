...
namespace GXPEngine.SceneManagement
{
    internal static class SceneManager
    {
        ...
        {
            var game = Game.main;
            if (CurrentScene != null) game.RemoveChild(CurrentScene);
            CurrentScene = scene;
            CurrentScene?.OnLoad();
        ...
        /// <summary>Prints all the scenes in the dictionary</summary>
        public static void PrintScenes()
        {
            foreach (var sceneName in scenes.Keys) Console.WriteLine($"Scene {scenes[sceneName]} with name {sceneName}");
        }
    }
}
